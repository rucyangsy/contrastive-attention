...
def main():
    ...
    for env_name in config.env_name:
        env = make_vec_envs(env_name, config.seed, config.num_processes,
    ...
    for j in range(num_updates):
        ...
        if j == 0:
            with torch.no_grad():
                value, action, action_log_prob, recurrent_hidden_states = actor_critic.act(
                    rollouts.obs[0], rollouts.recurrent_hidden_states[0],
                    rollouts.masks[0])   #rollouts.obs[step].shape = [16,4,84,84]

            recurrent_hidden_states, meta = recurrent_hidden_states

        for step in range(config.num_steps):
            # Sample actions
            for num in range(config.env_num): 
                    
                actions = torch.zeros((config.num_processes, 1), dtype=torch.int32)
                for action_num in range(config.num_processes):
                    actions[action_num] = int(envs[num].action_space.sample())
                # Obser reward and next obs
                actions.to(device)
                obs, reward, done, infos = envs[num].step(actions)
                objects_locs = []
        ...
        if config.train_selfsup_attention and j > 1001:
        ...
        if j % config.log_interval == 0 and config.train_selfsup_attention and j > 1001:
